...
const colorF_Background = '#FFFFFF';
const colorF_UnusedPiece = '#E0E0E0';
const colorS_UnusedPiece = '#BBBBBB';
const colorF_ProperPiece_Default = '#0080D0';
const colorF_NormalPiece_Default = '#004080';
const colorS_TargetPiece = '#BBBB00';
const colorF_TargetPiece = 'rgba(255, 255, 0, 0.5)';
const colorF_Point3 = 'rgba(255, 255, 255, 0.7)';
const colorS_Point3 = '#FF0000';
const colorF_Point2 = '#00CC00';
const colorS_Point2 = '#007700';
const colorS_Lozenge_Default = '#5599DD';
const colorF_UnusedPieceIndex_Default = '#000000';
const colorF_ProperPieceIndex_Default = '#FFFF00';
const colorF_NormalPieceIndex_Default = '#FFFF00';
let colorF_ProperPiece = colorF_ProperPiece_Default;
let colorF_NormalPiece = colorF_NormalPiece_Default;
let colorS_Lozenge = colorS_Lozenge_Default;
let colorF_UnusedPieceIndex = colorF_UnusedPieceIndex_Default;
let colorF_ProperPieceIndex = colorF_ProperPieceIndex_Default;
let colorF_NormalPieceIndex = colorF_NormalPieceIndex_Default;
...
function CalcIndexColorText(c1){
  ...
  let c2;
  let c = [0, 0, 0];
  c2 = [  0,   0,   0]; diff = calc_color_distance(c1, c2); if(diff > diffMax){ diffMax = diff; c = c2; }
  c2 = [255,   0,   0]; diff = calc_color_distance(c1, c2); if(diff > diffMax){ diffMax = diff; c = c2; }
  c2 = [  0, 255,   0]; diff = calc_color_distance(c1, c2); if(diff > diffMax){ diffMax = diff; c = c2; }
  c2 = [  0,   0, 255]; diff = calc_color_distance(c1, c2); if(diff > diffMax){ diffMax = diff; c = c2; }
  c2 = [  0, 255, 255]; diff = calc_color_distance(c1, c2); if(diff > diffMax){ diffMax = diff; c = c2; }
  c2 = [255,   0, 255]; diff = calc_color_distance(c1, c2); if(diff > diffMax){ diffMax = diff; c = c2; }
  c2 = [255, 255,   0]; diff = calc_color_distance(c1, c2); if(diff > diffMax){ diffMax = diff; c = c2; }
  c2 = [255, 255, 255]; diff = calc_color_distance(c1, c2); if(diff > diffMax){ diffMax = diff; c = c2; }
...
let hue_default = 210;
let hue_rad_default = (hue_default - 60) * Math.PI / 180 - Math.PI / 2;
let sat_default = 255;
let val_default = 128;
let hue_rad = hue_rad_default;
let hue_value = hue_default;
let sat_value = sat_default;
let val_value = val_default;
...
function onColorSelecting(e){
  ...
  if(f){
    let rgb = hsv2rgb(hue_value, sat_value, val_value);
    let rgb_proper = hsv2rgb(hue_value, sat_value / 2, (val_value + 255) / 2);
...
function DrawHSV(){
  ...
  function DrawSVRect(){
    ...
    let color = hsv2rgb(hue_value, 255, 255);
...
function onButtonClick_DefaultColor(){
  ctxColor.clearRect(0, 0, canvasForColor.width, canvasForColor.height);
